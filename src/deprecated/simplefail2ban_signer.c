...
void * unban_thread_routine(void * args)
{
	...
	**/
	...
	intervention_client_t *unbanning_client = intervention_client_create();
		...
		if(iterator != NULL)
		{
			...
			while(iterator != NULL)
			{
				...
				case AF_INET:
					...
					inet_ntop(AF_INET, iterator->key, ip_string, 16);
					printf("occurance of ipv4 %s in ip_hashtable: %d\n", ip_string, retval);
					const char *response = sendUnblockRequestv4(unbanning_client, ip_string);
					printf("answer of gRPC: %s\n", response);
					free((void*)response); // Why (void*) casting
...
void * ban_thread_routine(void * args)
{
	...
	**/
	...
	intervention_client_t *gRPC_Client = intervention_client_create();
			...
			for(i = 0; i < recv_retval; i++)
			{
				...
				if(retval == limit)
				{
					...
					case AF_INET:
						...
						char ip_string[16] = {'\0'};
						inet_ntop(AF_INET, &context.ip_addr.ipv4, ip_string, 16);
						const char *response = sendBlockRequestv4(gRPC_Client, ip_string);
						printf("answer of gRPC: %s\n", response);
						free((void*)response); // Why (void*) casting
